...
struct Hamiltonian {
  ...
  explicit Hamiltonian(const VT<PauliTerm<T>> &ham)
      : how_to_(ORIGIN), ham_(ham) {}

  Hamiltonian(const VT<PauliTerm<T>> &ham, Index n_qubits)
      : how_to_(BACKEND), n_qubits_(n_qubits), ham_(ham) {
    if (n_qubits_ > 16) {

...
struct ParameterResolver {
  ...
  ParameterResolver(const VT<std::string> &names, const VT<T> &coeffs,
                    const VT<bool> &requires_grads) {
...
template <typename T>
T LinearCombine(const ParameterResolver<T> &pr_big,
                const ParameterResolver<T> &pr_small) {
  T res = 0;
  for (typename MST<T>::const_iterator i = pr_small.data_.begin();
       i != pr_small.data_.end(); i++) {

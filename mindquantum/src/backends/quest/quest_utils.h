...
template <typename T>
inline ComplexMatrixN Dim2Matrix2ComplexMatrixN(const Dim2Matrix<T> &matrix,
                                                int n_qubits) {
...
#ifdef GPUACCELERATED
  cudaMemcpy(ori_real, qureg.deviceStateVec.real, dim * sizeof(qreal),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(ori_imag, qureg.deviceStateVec.imag, dim * sizeof(qreal),
             cudaMemcpyDeviceToHost);
#else
...
template <typename T>
inline void Csr_Dot_Vec(std::shared_ptr<CsrHdMatrix<T>> a,
                        std::shared_ptr<CsrHdMatrix<T>> b, Qureg qureg) {
  auto dim = a->dim_;
  auto vec_real = reinterpret_cast<qreal *>(malloc(sizeof(qreal) * dim));
  auto vec_imag = reinterpret_cast<qreal *>(malloc(sizeof(qreal) * dim));
  auto ori_real = reinterpret_cast<qreal *>(malloc(sizeof(qreal) * dim));

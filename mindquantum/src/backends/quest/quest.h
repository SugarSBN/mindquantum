...
struct Quest {
  ...
  QuESTEnv env;
  Qureg qubits;
  explicit Quest(int n) : n_qubits_(n) {
    env = createQuESTEnv();
    qubits = createQureg(n, env);
    initZeroState(qubits);
  }
  Quest(int n, Qureg ref_qureg) : n_qubits_(n) {
    env = createQuESTEnv();
    qubits = createQureg(n, env);
  ...
  Quest() : n_qubits_(1) {
    env = createQuESTEnv();
    qubits = createQureg(n_qubits_, env);
    initZeroState(qubits);
  }
  ~Quest() {
  ...
  void InitializeSimulator() { initZeroState(qubits); }
  void PrintInfo() {
  ...
  void ApplyGate(const BasicGate<T> &gate) {
    if (gate.ctrl_qubits_.size() == 0) {  // no control
      if (gate.name_ == gX) {
        ...
        rotateX(qubits, gate.obj_qubits_[0],
                static_cast<qreal>(gate.applied_value_));
      } else if (gate.name_ == gRY) {
        rotateY(qubits, gate.obj_qubits_[0],
                static_cast<qreal>(gate.applied_value_));
      } else if (gate.name_ == gRZ) {
        rotateZ(qubits, gate.obj_qubits_[0],
                static_cast<qreal>(gate.applied_value_));
      } else if (gate.name_ == gPS) {
        phaseShift(qubits, gate.obj_qubits_[0],
                   static_cast<qreal>(gate.applied_value_));
      } else {
        auto obj = Vec2Intptr(gate.obj_qubits_);
        auto m = Dim2Matrix2ComplexMatrixN<T>(gate.base_matrix_,
                                              gate.obj_qubits_.size());
        multiQubitUnitary(qubits, obj,
      ...
      if (gate.name_ == gX) {
        ...
        controlledMultiQubitUnitary(qubits, gate.ctrl_qubits_[0], obj,
                                    static_cast<int>(gate.obj_qubits_.size()),
                                    m);
        if (obj != nullptr) {
          free(obj);
        }
  ...
  void ApplyGate(const BasicGate<T> &gate, const ParameterResolver<T> &pr,
    ...
    if (diff) {
      ...
      if (nctrl == 0 && nobj == 1) {
        ...
        auto m = Dim2Matrix2ComplexMatrixN<T>(gate.param_diff_matrix_(theta),
                                              gate.obj_qubits_.size());
  ...
  // TODO(unknown): Stuff.🔥🔥🔥🔥🔥《模拟器开发》↪️开发quest模拟器的测量算符操作逻辑，GPU和CPU都需通过
  unsigned ApplyMeasure(const BasicGate<T> &gate) {}

  // TODO(unknown): Stuff.🔥🔥🔥🔥🔥《模拟器开发》↪️开发quest模拟器的采样方法逻辑，GPU和CPU都需通过
  ...
  VT<CT<T>> RightSizeGrad(Qureg left_vec, Qureg right_vec,
    ...
    for (size_t j = 0; j < circ.size(); j++) {
      if ((!herm_circ[j].parameterized_) ||
        ...
        gi = Complex2Complex<T>(
            calcInnerProduct(sim_left.qubits, sim_right_tmp.qubits));
        // } else {
        // TODO(unknown): Stuff.🔥🔥🔥🔥🔥《模拟器开发》↪️当含参门有控制位时如何进行内积运算
  ...
  VT<VT<CT<T>>> HermitianMeasureWithGrad(const VT<Hamiltonian<T>> &hams,
                                         ...
                                         const VT<BasicGate<T>> &herm_circ,
                                         const ParameterResolver<T> &pr,
                                         const VT<std::string> &params_order,
                                         size_t mea_threads) {
    ...
    for (size_t i = 0; i < n_hams; i++) {
      auto t2 = NOW();
      auto f_g = sim.RightSizeGrad(sim.qubits, sim.qubits, hams[i], circ,
                                   herm_circ, pr, p_map);
      auto t3 = NOW();
  ...
  VT<VT<VT<CT<T>>>> HermitianMeasureWithGrad(
    ...
    for (size_t i = 0; i < n_prs; i++) {
      auto f_g = HermitianMeasureWithGrad(hams, circ, herm_circ, prs[i],
                                          params_order, mea_threads);
      output[i] = f_g;
    }
    return output;
  }

...
class Projectq : public Simulator {
 ...
 public:
  ...
  Projectq(unsigned seed, unsigned N)
      : Simulator(seed, N), n_qubits_(N), rnd_eng_(seed) {
    for (unsigned i = 0; i < n_qubits_; i++) {
      ordering_.push_back(i);
    }
    len_ = (1UL << (n_qubits_ + 1));
  ...
  Projectq(unsigned seed, unsigned N, calc_type *vec)
      : Simulator(seed, N), n_qubits_(N), rnd_eng_(seed) {
    for (unsigned i = 0; i < n_qubits_; i++) {
      ordering_.push_back(i);
    }
  ...
  void ApplyGate(const BasicGate<T> &gate, const ParameterResolver<T> &pr,
                 bool diff = false) {
    T theta = LinearCombine(pr, gate.params_);
    if (diff) {
      Projectq::apply_controlled_gate(
          MCast<T>(gate.param_diff_matrix_(theta).matrix_),
          VCast(gate.obj_qubits_), VCast(gate.ctrl_qubits_));
    } else {
      Projectq::apply_controlled_gate(
          MCast<T>(gate.param_matrix_(theta).matrix_), VCast(gate.obj_qubits_),
          VCast(gate.ctrl_qubits_));
    }
  }
  ...
  unsigned ApplyMeasure(const BasicGate<T> &gate) {
    ...
    for (unsigned i = 0; i < (len_ >> 1); i++) {
      if ((i & mask) == 0) {
        zero_amps +=
            vec_[2 * i] * vec_[2 * i] + vec_[2 * i + 1] * vec_[2 * i + 1];
      }
    }
...
#pragma omp parallel for schedule(static)
  ...
  VT<unsigned> Sampling(const VT<BasicGate<T>> &circ,
                        const ParameterResolver<T> &pr, size_t shots,
    ...
    for (size_t i = 0; i < shots; i++) {
      ...
      for (auto &gate : circ) {
        if (gate.is_measure_) {
          auto collapse = sim.ApplyMeasure(gate);
          res[i * key_size + key_map.at(gate.name_)] = collapse;
        } else if (gate.parameterized_) {
          sim.ApplyGate(gate, pr);
        } else {
          sim.ApplyGate(gate);
        }
      }
  ...
  void ApplyHamiltonian(const Hamiltonian<T> &ham) {
    ...
    if (ham.how_to_ == ORIGIN) {
      ...
      Projectq::vec_ = sparse::Csr_Dot_Vec<T, double>(
          ham.ham_sparse_main_, ham.ham_sparse_second_, Projectq::vec_);
    } else {
      Projectq::vec_ =
          sparse::Csr_Dot_Vec<T, double>(ham.ham_sparse_main_, Projectq::vec_);
    }
  }

  VT<CT<T>> RightSizeGrad(calc_type *left_vec, calc_type *right_vec,
                          const Hamiltonian<T> &ham,
                          const VT<BasicGate<T>> &circ,
                          const VT<BasicGate<T>> &herm_circ,
                          const ParameterResolver<T> &pr,
    ...
    f_g[0] = ComplexInnerProduct<T, calc_type>(sim_left.vec_, right_vec,
    ...
    for (size_t j = 0; j < circ.size(); j++) {
      if ((!herm_circ[j].parameterized_) ||
          (herm_circ[j].params_.requires_grad_parameters_.size() == 0)) {
        if (herm_circ[j].parameterized_) {
        ...
        if (herm_circ[j].ctrl_qubits_.size() == 0) {
          gi = ComplexInnerProduct<T, calc_type>(
              sim_left.vec_, sim_right_tmp.vec_, static_cast<Index>(len_));
        } else {
  ...
  CT<T> GetExpectation(const Hamiltonian<T> &ham) {
    ...
    auto out = ComplexInnerProduct<T, calc_type>(sim.vec_, vec_,
                                                 static_cast<Index>(len_));
    return out;
  }

  VT<VT<CT<T>>> HermitianMeasureWithGrad(const VT<Hamiltonian<T>> &hams,
                                         const VT<BasicGate<T>> &circ,
                                         const VT<BasicGate<T>> &herm_circ,
                                         const ParameterResolver<T> &pr,
                                         const MST<size_t> &p_map,
    ...
    if (n_hams == 1) {
      auto f_g = sim.RightSizeGrad(sim.vec_, sim.vec_, hams[0], circ, herm_circ,
                                   pr, p_map);
      ...
      for (size_t i = 0; i < mea_threads; ++i) {
        ...
        auto task = [&, start, end]() {
          for (size_t n = start; n < end; n++) {
            auto f_g = sim.RightSizeGrad(sim.vec_, sim.vec_, hams[n], circ,
                                         herm_circ, pr, p_map);
            for (size_t g = 1; g < n_params + 1; g++) {
              f_g[g] += std::conj(f_g[g]);
            }
  ...
  VT<VT<VT<CT<T>>>> HermitianMeasureWithGrad(
      const VT<Hamiltonian<T>> &hams, const VT<BasicGate<T>> &circ,
    ...
    if (n_prs == 1) {
      ...
      output[0] = HermitianMeasureWithGrad(hams, circ, herm_circ, pr, p_map,
                                           mea_threads);
    } else {
      std::vector<std::thread> tasks;
      ...
      for (size_t i = 0; i < batch_threads; ++i) {
        ...
        auto task = [&, start, end]() {
          for (size_t n = start; n < end; n++) {
            ...
            pr.SetData(ans_data, ans_name);
            auto f_g = HermitianMeasureWithGrad(hams, circ, herm_circ, pr,
                                                p_map, mea_threads);
            output[n] = f_g;
          }
        };
  ...
  VT<VT<CT<T>>> NonHermitianMeasureWithGrad(
      const VT<Hamiltonian<T>> &hams, const VT<Hamiltonian<T>> &herm_hams,
    ...
    if (n_hams == 1) {
      auto f_g1 = sim2.RightSizeGrad(sim2.vec_, sim.vec_, herm_hams[0],
                                     right_circ, herm_right_circ, pr, p_map);
      auto f_g2 = sim.RightSizeGrad(sim.vec_, sim2.vec_, hams[0], left_circ,
                                    herm_right_circ, pr, p_map);
      for (size_t g = 1; g < n_params + 1; g++) {
        f_g1[g] += std::conj(f_g2[g]);
      }
      output[0] = f_g1;
    } else {
      std::vector<std::thread> tasks;
      ...
      for (size_t i = 0; i < mea_threads; i++) {
        ...
        auto task = [&, start, end]() {
          for (size_t n = start; n < end; n++) {
            auto f_g1 =
                sim2.RightSizeGrad(sim2.vec_, sim.vec_, herm_hams[n],
                                   right_circ, herm_right_circ, pr, p_map);
            auto f_g2 =
                sim.RightSizeGrad(sim.vec_, sim2.vec_, hams[n], left_circ,
                                  herm_right_circ, pr, p_map);
            for (size_t g = 1; g < n_params + 1; g++) {
              f_g1[g] += std::conj(f_g2[g]);
            }
  ...
  VT<VT<VT<CT<T>>>> NonHermitianMeasureWithGrad(
    ...
    if (n_prs == 1) {
      ...
      pr.SetData(enc_data[0], enc_name);
      pr.SetData(ans_data, ans_name);
      output[0] = NonHermitianMeasureWithGrad(
          hams, herm_hams, left_circ, herm_left_circ, right_circ,
      ...
      for (size_t i = 0; i < batch_threads; ++i) {
        ...
        auto task = [&, start, end]() {
          for (size_t n = start; n < end; n++) {
            ...
            pr.SetData(enc_data[n], enc_name);
            pr.SetData(ans_data, ans_name);
            auto f_g = NonHermitianMeasureWithGrad(
                hams, herm_hams, left_circ, herm_left_circ, right_circ,
                herm_right_circ, pr, p_map, mea_threads);
            output[n] = f_g;
          }
  ...
  void PrintInfo() {
    std::cout << n_qubits_
              << " qubits simulator with currently quantum state at:"
              << std::endl;
    for (unsigned i = 0; i < (len_ >> 1); i++) {
      std::cout << "(" << vec_[2 * i] << ", " << vec_[2 * i + 1] << ")"
                << std::endl;
    }
  }
};

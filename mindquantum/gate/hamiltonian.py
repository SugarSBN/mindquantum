...
class Hamiltonian:
    ...
    def __init__(self, hamiltonian):
        ...
        if not isinstance(hamiltonian, support_type):
            raise TypeError(
                "Require a QubitOperator or a csr_matrix, but get {}!".format(
                    type(hamiltonian)))
        if isinstance(hamiltonian, sp.csr_matrix):
            if len(hamiltonian.shape
                   ) != 2 or hamiltonian.shape[0] != hamiltonian.shape[1]:
                raise ValueError(
            ...
            if np.log2(hamiltonian.shape[0]) % 1 != 0:
                raise ValueError(
                    f"size of hamiltonian sparse matrix should be power of 2, but get {hamiltonian.shape}"
                )
